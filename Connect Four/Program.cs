...
void GameLoop()
{
    while (true)
    {
        ...
        gameboard.CheckForVictory(Player.Player1);
...
public class GameBoard
{
    ...
    public bool VerticalValidation(int column)
    {
        try
        {
            Tokens.Find(x => x.Column == column && x.Row == 6 && x.Player == Player.None);
        }
        catch (Exception)
        {
            if (currentPlayer == Player.Player1)
    ...
    public void GetGameBoard()
    {
        ...
        Console.WriteLine($"Current Round: {Turn}    Difficulty: {Difficulty}");
        Console.WriteLine("-----------------------------------------");
    }
